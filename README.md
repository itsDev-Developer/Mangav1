...
<details>
    <summary><b>Click Here</b></summary>
    <ol>
        <li>Run <code>/postsettings</code> (admin only) and set your <b>Dump Channel</b> (where files are stored — set it by <i>forwarding</i> a message from that channel, so it's stored as a numeric ID) and <b>Post Channel</b> (where the formatted post is published). The bot must be an admin in both.</li>
        <li><b>Manual:</b> run <code>/newpost</code> (or tap "➕ Create New Post"). The bot asks you to send the file(s) — saved straight into the Dump Channel — then the poster image, title, genre and description.</li>
        <li><b>Automatic:</b> <code>/search</code> for a manga as usual, open a result, and tap "📤 Post to Channel" (only admins see this button). The bot fetches the title/genre/description straight from the site, lets you pick which chapters (all / latest / a custom range like <code>1-20</code>), downloads and dumps them into the Dump Channel, and auto-builds the batch of file(s) behind the button — no typing required.</li>
        <li>Either way you land on the same preview. Review it and tap "✅ Confirm & Publish". The post goes live in your Post Channel with a "📖 Read Now" button.</li>
        <li>When a user taps "📖 Read Now", the bot opens a private chat with them and sends the linked file(s) (the "batch") straight from the Dump Channel.</li>
        <li>Manage or delete existing posts any time from "📋 Manage Posts" inside <code>/postsettings</code>.</li>
    </ol>
    <b>Note:</b> genre/description/status are parsed from what each site module already scrapes for the search/info screens — a rating field isn't currently scraped by any site module, so it's left out of the auto-filled preview.
</details>
